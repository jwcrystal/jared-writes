...
六個致命問題構成一個很小的 bug 群 — 崩潰、卡死、hang。這類問題通常不是功能邏輯錯誤，而是 edge case 的 exception handling 沒做好。修起來可能很快（每個幾行 code），但影響巨大。
...
主要是 UI 渲染、PWA 相容、編輯體驗的細節問題：檔案樹不刷新（#836）、@mention 刪除行為（#792）、VSCode 擴展相容（#846）、Reasoning level 重置（#748）等。
