...
Live555 和 ZLMediaKit 都是 C++ 寫的開源流媒體解決方案，但定位完全不同。Live555 是一個經典的 RTSP 流媒體庫，適合學術研究和嵌入式設備，協議支援窄（RTSP/RTP）、部署複雜。ZLMediaKit 是全協議流媒體服務框架，同時支援 RTSP、RTMP、WebRTC、SRT、GB28181，效能更高、文件更好、授權更寬鬆（MIT），適合商業應用和大規模部署。
...
- 如果你的需求只是播 RTSP 流，Live555 可以做到，但 ZLMediaKit 也可以做到而且更容易。除非硬體資源極度受限（嵌入式 IoT），否則沒有理由選 Live555。
