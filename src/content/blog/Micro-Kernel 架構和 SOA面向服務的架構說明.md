---
title: Micro-Kernel 架構和 SOA（面向服務的架構）說明
...
# Micro-Kernel 架構和 SOA 說明

## 核心摘要

Micro-Kernel 和 SOA 是兩個容易混淆的架構模式，但它們解決的是完全不同的問題。Micro-Kernel 是**單一應用內部的模組化策略**——核心小而穩定，功能透過 plugin 動態掛載。SOA 是**跨應用、跨服務的組織策略**——每個服務獨立部署、獨立演化，透過標準化介面（REST/SOAP）互連。記住這個區別：Micro-Kernel 是一個 app 的內部設計；SOA 是多個 app 之間的外部協定。
...
每個服務獨自擁有業務邏輯和資料，透過標準化介面（ESB、REST、SOAP）溝通。強調服務的獨立性、可重用性、跨應用的組合。
...
- Micro-Kernel 最適合**產品型軟體**：你需要一個穩定的核心產品，但不同客戶要不同的功能組合。Plugin 架構讓你可以賣 core + 選配。VS Code 就是用這個模式做到極致的例子。
- SOA 最適合**大型組織**：不同團隊負責不同服務，需要標準化介面來降低協作摩擦。但 SOA 的歷史包袱很重——ESB 時代的 SOA 太厚重，後來被微服務取代了大部分場景。
