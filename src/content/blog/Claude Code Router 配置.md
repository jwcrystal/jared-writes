...
如果你對隱私有要求，可以啟用 ZDR，請求只會路由到支援零資料留存的 endpoint。ZDR 模式下的推薦組合略有不同：
...
每組路由建議至少配置 2 個 provider 做 fallback，避免單一供應商掛掉時服務中斷：
...
- Router 的配置**不是設一次就不用管的**——model 定價一直在變，每個月至少要 check 一次推薦組合。
