...
MySQL 的架構分四層：連接層管連線和認證，服務層做解析、優化和執行，儲存引擎層負責實際資料的讀寫（InnoDB 是預設王者），檔案系統層把資料落地到磁盤。理解這四層的分工，就理解了為什麼一條 SQL 從客戶端送到 MySQL 後要經過這麼多步驟 — 以及為什麼說 MySQL 的性能瓶頸通常在儲存引擎層。
