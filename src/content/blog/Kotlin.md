...
Kotlin 協程在 Android 開發中有兩個核心機制需要掌握：`lifecycleScope.launch(Dispatchers.IO)` 和 `suspend` 函數。前者解決的是**在哪個執行緒跑、何時自動取消**（配合 Activity/Fragment 生命週期）。後者解決的是**非同步程式碼寫起來像同步**（暫停不阻塞主執行緒）。兩者結合的結果是：你可以在 IO 執行緒做網路請求，完成後自動切回 Main 執行緒更新 UI，而且 Activity 銷毀時協程自動取消。
...
不加 `Dispatchers.IO` 的話協程雖然是非同步的，但**仍然在主執行緒執行**——長時間操作一樣會卡 UI。
...
- suspend 函數的核心價值不是「非同步」，而是**讓非同步程式碼長得像同步**。這大幅降低了回調地獄和狀態管理的複雜度。
